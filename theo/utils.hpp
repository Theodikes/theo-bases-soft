...
/* Обработка любого файла оптимальными для чтения чанками (с записью чанков во временный буфер).
...
* входного буфера записываются в resultFile. 
* Внимание, функция закрывает inputFile после полной обработки, но resultFile остаётся открытым! */
